...
from bluuberrylibrary.utils.instances.bb_buff_utils import BBBuffUtils
...
class BBSimBuffUtils:
    ...
    def add_buff(cls, sim_info: SimInfo, buff: Union[int, Buff], reason: int) -> BBRunResult:
        ...
        if buff_instance is None:
            return BBRunResult(False, f'No buff existed with GUID {buff}.')
        ...
        if added is None:
            return BBRunResult(False, f'Failed to add buff {buff_instance} to Sim {sim_info}.')
        return BBRunResult(True, f'Successfully added buff {buff_instance} to Sim {sim_info}.')

    @classmethod
    def can_add_buff(cls, sim_info: SimInfo, buff: Union[int, Buff]) -> BBRunResult:
        ...
        buff_instance = BBBuffUtils.load_buff_by_guid(buff)
        if buff_instance is None:
            return BBRunResult(False, f'Cannot add buff to Sim {sim_info}. No buff existed with GUID {buff}.')
        sim = BBSimUtils.to_sim_instance(sim_info)
        if sim is None:
            return BBRunResult(False, f'Cannot add buff {buff_instance} to Sim {sim_info} because they are not currently spawned.')
        # noinspection PyUnresolvedReferences
        buff_add_test_set = buff_instance._add_test_set
        if buff_add_test_set is not None:
            resolver = SingleSimResolver(sim)
            result: BBRunResult = buff_add_test_set.run_tests(resolver)
            if not result:
                return BBRunResult(False, f'Cannot add buff {buff_instance}. The tests on it failed for Sim {sim_info} because reason "{result.reason}".')
        # noinspection PyUnresolvedReferences
        ...
        if mood is not None and mood.excluding_traits is not None and sim_info.trait_tracker.has_any_trait(mood.excluding_traits):
            return BBRunResult(False, f'Cannot add buff {buff_instance}. The Mood {mood} on the buff is not allowed for the Sim {sim_info}. Mood Excluding Traits: {mood.excluding_traits}')
        if buff_instance.exclusive_index is None:
            return BBRunResult(True, f'Can add buff {buff_instance} to Sim {sim_info} it has no conflicts.')
        for conflicting_buff_type in sim_info.buff_component._active_buffs:
            ...
            if conflicting_buff_type.exclusive_index == buff_instance.exclusive_index:
                if buff_instance.exclusive_weight < conflicting_buff_type.exclusive_weight:
                    return BBRunResult(False, f'Cannot add buff {buff_instance} because it conflicts with buff {conflicting_buff_type} on Sim {sim_info}.')
        return BBRunResult(True, f'Can add buff {buff_instance} to Sim {sim_info}.')

    @classmethod
    def remove_buff(cls, sim_info: SimInfo, buff: Union[int, Buff]) -> BBRunResult:
        ...
        buff_instance = BBBuffUtils.load_buff_by_guid(buff)
        if buff_instance is None:
            return BBRunResult(False, f'Cannot remove buff from Sim {sim_info}. No buff existed with GUID {buff}.')
        sim_info.remove_buff_by_type(buff_instance)
        return BBRunResult(True, f'Successfully removed Buff {buff_instance} from Sim {sim_info}.')
